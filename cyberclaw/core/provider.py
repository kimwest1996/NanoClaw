...
OPENAI_COMPATIBLE_PROVIDERS = {"openai", "aliyun", "dashscope", "z.ai", "tencent", "other"}


def _resolve_required_key(explicit_key: str | None, env_key: str, missing_message: str) -> str:
    value = explicit_key or os.environ.get(env_key)
    if not value:
...
def get_provider(
    ...
    **kwargs: Any
    ...
    if provider_name in OPENAI_COMPATIBLE_PROVIDERS:
        ...
        current_api_key = _resolve_required_key(
            explicit_key=api_key,
            env_key="OPENAI_API_KEY",
            missing_message="未找到 API Key！请确保 .env 中配置了 OPENAI_API_KEY",
        )
        ...
        current_api_key = _resolve_required_key(
            ...
            missing_message="未找到 ANTHROPIC_API_KEY 环境变量！",
        )
