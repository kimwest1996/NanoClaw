...
> 🤖 **你的 AI 在背着你做什么？NanoClaw 让所有行为无所遁形**
> 
> 💡 **灵感来源**：受 [OpenClaw](https://github.com/openclaw/openclaw) 的启发，NanoClaw 专注于解决 AI 智能体的透明度和可控性问题。

---

## 📖 简介

NanoClaw 是一个**企业级透明可控智能体**，重新定义 AI 系统的可信边界：
...
NanoClaw 支持**OpenClaw 技能**和**Claude Code 技能**，可直接使用两个生态系统的丰富技能资源，无需重新开发。
...
- **推荐技能**：
  - `skill-creator`：用自然语言让 NanoClaw 自己创建技能
...
- [NanoClaw Improvement Directions](docs/improvement-directions.md)
...
> 💡 **工作区配置**：工作区路径已在代码中初始化，默认为项目根目录的 `workspace` 文件夹，无需在 `.env` 中配置。仅当需要自定义工作区位置时，才设置 `NANOCLAW_WORKSPACE` 环境变量。

> 💡 提示：配置完成后，可运行 `nanoclaw run` 聊天测试连接是否正常。
...
启动后进入交互式对话界面，可直接通过自然语言发起时间查询、任务调度、文件操作和技能调用。
...
| 模块 | 文件 | 功能 |
|------|------|------|
| **Agent 循环** | `nanoclaw/core/agent.py` | LangGraph StateGraph，决策大脑 |
| **技能加载** | `nanoclaw/core/skill_loader.py` | 动态加载 SKILL.md，两段式调用 |
| **上下文管理** | `nanoclaw/core/context.py` | 消息修剪，双水位记忆 |
| **内置工具** | `nanoclaw/core/tools/builtins.py` | 时间/计算/任务调度等 |
| **沙盒工具** | `nanoclaw/core/tools/sandbox_tools.py` | 文件操作 + Shell 执行 |
| **审计日志** | `nanoclaw/core/logger.py` | JSONL 格式事件记录 |
| **心跳任务** | `nanoclaw/core/heartbeat.py` | 定时任务检查与触发 |
...
├── docs/                         # 项目文档
│   └── improvement-directions.md # 改造方向与研究结论
├── entry/
│   ├── main.py                   # 主程序入口
│   ├── cli.py                    # CLI 配置向导
│   └── monitor.py                # 监控终端
...
- **GitHub**: [@kimwest1996](https://github.com/kimwest1996)
- **邮箱**: thor07@126.com
...
[![Star History Chart](https://api.star-history.com/svg?repos=kimwest1996/NanoClaw&type=Date)](https://star-history.com/#kimwest1996/NanoClaw&Date)
...
Made with ❤️ by [@kimwest1996](https://github.com/kimwest1996)
